...
def dicom_to_nifti(series_dir, is_seg=False):
    ...
    print(f"\ndicom_to_nifti latency : {time.time() - start} (sec)")
    return output_file


def binary_to_image(reference_image, label, dtype=np.uint16, file_ext=".nii.gz", use_itk=False):
    start = time.time()

    image_np, meta_dict = LoadImage()(reference_image)
    label_np = np.fromfile(label, dtype=dtype)

    print(f"\nImage: {image_np.shape}")
    print(f"\nLabel: {label_np.shape}")
    ...
    print(f"\nbinary_to_image latency : {time.time() - start} (sec)")
    return output_file


def nifti_to_dicom_seg(series_dir, label, label_info, file_ext="*", use_itk=False):
    start = time.time()
    ...
    if not segment_attributes:
        print("\nMissing Attributes/Empty Label provided")
        return None
    ...
    print(f"\nnifti_to_dicom_seg latency : {time.time() - start} (sec)")
    return output_file
...
def itk_dicom_seg_to_image(label, output_type="nifti"):
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        if not output_files:
            print(f"Failed to convert DICOM-SEG {label} to NIFTI")
            return None

        result_file = os.path.join(output_dir, output_files[0])
        print(f"Result/Output (NII) File: {result_file}")
